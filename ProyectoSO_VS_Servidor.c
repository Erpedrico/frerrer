...
int main(int argc, char *argv[])
{
	...
	for (;;){
		...
		while (terminar ==0)
		{
			...
			{	
				char consulta [80];
				strcpy (consulta,"SELECT Usuarios.Contraseña FROM Usuarios WHERE Usuarios.nombre = '");
				...
				result= strcmp(resultado, contraseña);
				if (result == 0)
			...
			{
				char consulta [80];
				strcpy (consulta,"SELECT Partidas.Duración FROM Partidas WHERE Partidas.id= '");
				strcat (consulta, partida);
				strcat (consulta,"'");
